...
class Application:
    ...
    def enc(self):
        self.__root.withdraw()
        source, name, size = self.__get_source(False)
        if source:
            ...
            if destination:
                ...
                if major:
                    ...
                    spice_coding.encode(major, minor, source, \
                                        file(destination, 'wb'), size)
        self.__clean_screen()
        self.__root.deiconify()

    def dec(self):
        self.__root.withdraw()
        source, name, size = self.__get_source(True)
        if source:
            print 'Select the destination folder.'
            destination = self.__fold.show()
            if destination:
                ...
                if major:
                    ...
                    destination = os.path.join(destination, name)
                    spice_coding.decode(major, minor, source, \
                                        file(destination, 'wb'), size)
        self.__clean_screen()
        self.__root.deiconify()
            ...
            try:
                if filename:
                    ...
                    if decrypt:
                        ...
                        assert filesize <= self.LIMIT_SIZE, \
                               'File size exceeds ' + \
                               str(self.LIMIT_SIZE) + \
                               ' bytes.'
                    return file(filename, 'rb'), filename, filesize
