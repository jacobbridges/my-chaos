...
class Key(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.__repr = '%s(%s)' % (self.__class__.__name__, ', '.join('%s=%r' % (key, kwargs[key]) for key in kwargs))
        key = kwargs.pop('key', None)
        sub_key = kwargs.pop('sub_key', None)
        sam = kwargs.pop('sam', None)
        computer_name = kwargs.pop('computer_name', None)
    ...
    def load(self, sub_key, file_name):
        ...
        _winreg.LoadKey(self.__key, sub_key, file_name)
...
class Keys(object):
    ...
    DIR = _os.path.dirname(_sys.argv[0])
    ...
    def __getitem__(self, key):
        ...
        return Key(key=self.__key, sub_key=key)

    def __setitem__(self, key, value):
        'Assign the item to a key.'
        file_name = _os.path.join(DIR, ''.join(_random.sample(CHR, 8)))
        while _os.path.exists(file_name):
            file_name = _os.path.join(DIR, ''.join(_random.sample(CHR, 8)))
        try:
            value.save(file_name)
            _winreg.LoadKey(self.__key, key, file_name)
        finally:
    ...
    def __iter__(self):
        ...
        try:
            while True:
                keys.append(_winreg.EnumKey(self.__key, index))
                index += 1
        except EnvironmentError:
            for key in keys:
                yield key
...
class Values(object):
    ...
    def __iter__(self):
        ...
        try:
            while True:
                values.append(_winreg.EnumValue(self.__key, index)[0])
                index += 1
        except EnvironmentError:
            for value in values:
                yield value
