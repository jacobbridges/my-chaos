...
class Application:                  # this is the main class / [function]
    ...
    def __init__(self):
        ...
        self.__root.title('DAM 1.0')   # give it a name
        self.__root.resizable(False, False) # disable resizing

        # starts the splitter engine
        split = Tkinter.Button(self.__root, text='Divide', font='Courier 8', command=self.split)
        split.grid(row=0, column=0, padx=15, pady=5)

        # starts the joiner engine
        join = Tkinter.Button(self.__root, text='Merge', font='Courier 8', command=self.join)
