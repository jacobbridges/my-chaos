...
def main():
    try:
        ...
        if os.path.exists(destination):
            assert os.path.isdir(destination), '<destination_directory> is not a directory'
        else:
            os.makedirs(destination)
        copy(source, destination)
    except Exception, error:
        program = 'USAGE:  %s <source_directory> <destination_directory>' % os.path.basename(sys.argv[0])
    ...
    for name in os.listdir(source):
        ...
        try:
            if os.path.isdir(source_name):
                ...
                file(destination_name, 'wb').write(file(source_name, 'rb').read())
        except:
