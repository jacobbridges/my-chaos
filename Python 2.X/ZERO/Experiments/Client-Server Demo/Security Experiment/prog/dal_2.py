...
class DAL2:
    ...
    def read(self, block):
        ...
        return self.__disk.read(self.__index + block * self.__disk._DAL1__size, \
                                self.__disk._DAL1__size)

    # Write A Block
    def write(self, block, data):
        assert type(block) is int and 0 <= block < self.__blocks
        assert type(data) is str and len(data) == self.__disk._DAL1__size
        assert self.__BIT[block] != 0
        self.__disk.write(self.__index + block * self.__disk._DAL1__size, data)

    # Close A Block
    def close(self, block):
        assert type(block) is int and 0 <= block < self.__blocks
        assert self.__BIT[block] != 0
        self.__disk.erase(self.__index + block * self.__disk._DAL1__size, \
                          self.__disk._DAL1__size)
        self.__BIT[block] = 0
    ...
    def load(self, name, abstract):
        ...
        self.__blocks = (self.__disk._DAL1__blocks * self.__disk._DAL1__size) / \
                        (self.__disk._DAL1__size + 1)
        self.__index = (self.__disk._DAL1__blocks - self.__blocks) * self.__disk._DAL1__size
