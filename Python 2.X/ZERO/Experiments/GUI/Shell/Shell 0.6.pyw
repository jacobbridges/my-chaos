...
##    event.widget.quit()
    winreg = get_key(HKEY.CURRENT_USER, 'Software\\Atlantis Zero\\Demo Shell\\winreg', KEY.ALL_ACCESS)
    del winreg.values
    ...
    for index, item in enumerate(media.find_all()):
        ...
        winreg.values['IMAGE_%s' % (index + 1)] = REG_BINARY(cPickle.dumps(temp))
    event.widget.quit()

def get_key(hkey, sub_key, sam=None):
    key = Key(key=hkey)
